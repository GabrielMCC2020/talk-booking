...
def get_current_task_definition(client, cluster, service):
    ...
    return
    client.describe_task_definition(taskDefinition=current_task_arn)[
    "taskDefinition"
    ]

def create_new_task_definition(client, 
                               current_definition, 
                               new_image): container_definition = current_definition["containerDefinitions"][0].copy()
    ...
    def update_service(client, cluster, service, task_arn): client.update_service(
        cluster=cluster,
        service=service,
    ...
    while cnt < timeout: response = client.describe_services(cluster=cluster,
services=[service])
    print(response)
    deployment = next(
depl
for depl in response["services"]
[0]["deployments"]
if depl["status"] == "PRIMARY"
)
    if deployment["runningCount"] == deployment["desiredCount"]: deployment_finished = True
    break

    print(
f"Waiting ... Running count:
...
f"Desired count: {deployment['desiredCount']}"
    ...
    if __name__ == "__main__": DEPLOYMENT_TIMEOUT = 1800
...
ecs_client, cluster=cluster_name,
...
ecs_client, task_definition, new_image_uri
    ...
    if not finished: print("Did not stabilize ...")
    exit(1)
